...
export default function ContactWidget() {
  ...
  // Capture campaign tags from the landing URL once, and keep them for the
  // whole session — the visitor usually browses a few pages before opening
  // the form, by which point the ?utm_* params are long gone from the URL.
  useEffect(() => {
    const KEY = 'tb_utm'
    try {
      ...
      // private browsing / storage blocked — tracking is optional, carry on
    }
  }, [])
          ...
          >
            ...
            {done ? (
              ...
              <form onSubmit={submit} style={{ padding: '20px 20px 24px' }}>
                ...
                <div style={{ marginBottom: 16 }}>
                  <label htmlFor="cw-branch" style={labelStyle}>סניף מועדף <span style={{ color: '#8a7fa5', fontWeight: 500 }}>(לא חובה)</span></label>
                  <select id="cw-branch" value={branch} onChange={e => setBranch(e.target.value)} style={field}>
                    <option value="">בחרו סניף…</option>
                    {LEAD_BRANCHES.map(b => <option key={b} value={b}>{b}</option>)}
                  </select>
                </div>
