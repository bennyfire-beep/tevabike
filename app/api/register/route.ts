...
import { sendRegistrationConfirmationTemplate } from '@/lib/whatsapp-templates'
...
export async function POST(req: Request) {
  try {
    ...
    // WhatsApp confirmation template — opt-in only, and never allowed to
    // block or slow down the response to the parent. No-ops safely until
    // WHATSAPP_REGISTRATION_TEMPLATE_NAME is configured (see lib/whatsapp-templates).
    if (whatsappConsent && inserted) {
      const firstName = (body.full_name || '').trim().split(/\s+/)[0] || body.full_name
      sendRegistrationConfirmationTemplate(body.phone, firstName)
        .then(async (result) => {
          if (result.ok) {
            await supabase
              .from('registrations')
