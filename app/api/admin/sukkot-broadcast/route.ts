...
// שליחת הודעה לכל הנרשמים למחנה סוכות.
...
function pick(regs: Reg[], audience: Audience) {
  if (audience === 'paid') return regs.filter(r => r.payment_status === 'paid')
  if (audience === 'pending') return regs.filter(r => r.payment_status === 'pending')
  return regs
}

function html(r: Reg, title: string, bodyText: string, showPayButton: boolean) {
  ...
  const pay = showPayButton
    ? `<p style="margin:22px 0">
         <a href="${PAY_URL}" style="background:#b5e853;color:#0d0f0e;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:700;display:inline-block">להשלמת התשלום</a>
       </p>`
    : ''
  ...
  return `<div dir="rtl" style="font-family:Heebo,Arial,sans-serif;color:#1a1a1a;max-width:600px">
    ...
    <p style="margin:0 0 20px;color:#555">${CAMP_NAME} · ${CAMP_DATES}</p>
    ${greeting}
    ${paragraphs}
    ${pay}
...
async function sendEmail(to: string[], subject: string, body: string) {
  ...
  try {
    ...
    if (!res.ok) { console.error('[broadcast] resend failed:', res.status, await res.text()); return false }
    return true
  } catch (err) {
...
export async function POST(req: NextRequest) {
  ...
  let body: { subject?: string; message?: string; audience?: string; includePayButton?: boolean; testTo?: string }
  ...
  const { data, error } = await db
    .from('sukkot_registrations')
    .select('id, rider_first_name, rider_last_name, parent_name, parent_email, payment_status')
    .neq('payment_status', 'cancelled')
  ...
  for (const r of targets) {
    const ok = await sendEmail([r.parent_email], subject, html(r, subject, message, showPay))
    if (ok) sent += 1
    else failed.push(`${r.rider_first_name} ${r.rider_last_name} (${r.parent_email})`)
  }

  return NextResponse.json({ ok: true, sent, total: targets.length, failed })
}
